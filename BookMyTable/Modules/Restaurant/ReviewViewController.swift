...
//  BookMyTable
//
//  Created by Evelina on 24.05.2023.
//
...
//        image.layer.cornerRadius = 10
    ...
    private func setupView() {
        ...
        [restaurantView, reviewTableView].forEach({view.addSubview($0)})
        setupSubview()
    }
    
    private func setupSubview() {
        restaurantView.image = darkenImage(image: UIImage(data: restaurantImage ?? Data()) ?? UIImage())
        [reviewLabel, backButton].forEach({restaurantView.addSubview($0)})
        
        reviewTableView.delegate = self
        reviewTableView.dataSource = self
        reviewTableView.register(ReviewTableViewCell.self, forCellReuseIdentifier: String(describing: ReviewTableViewCell.self))
...
extension ReviewViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: String(describing: ReviewTableViewCell.self), for: indexPath)
                        as? ReviewTableViewCell else { return ReviewTableViewCell()}
